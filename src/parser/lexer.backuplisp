...
; (defun lex-list (l)
 ...
 ; (mapcar (lambda (x)
	    ; (cond ((string= x "forall") (make-token "forall"))
		  ; ((string= x "exists") (make-token "exists"))
		  ; ((string= x "and") (make-token "and"))
		  ; ((string= x "=>") (make-token "=>"))
		  ; ((string= x "<=") (make-token "<="))
		  ; ((string= x ".") (make-token "end"))
		  ; ((string= x ":") (make-token "in"))
...
(defun lex-list (l)
  "replaces strings in 'l' with appropriate tokens"
  (mapcar (lambda (x)
	    (cond ((string= x "forall") (make-token "resolution::forall"))
		  ((string= x "exists") (make-token "resolution::exists"))
		  ((string= x "and") (make-token "resolution::and"))
		  ((string= x "=>") (make-token "resolution::=>"))
		  ((string= x "<=") (make-token "resolution::<="))
		  ((string= x ".") (make-token "resolution::end"))
		  ((string= x ":") (make-token "resolution::in"))
		  ((= (length x) 1) (make-variable-sym x))
		  (t (make-literal-sym x))))
	  l))
