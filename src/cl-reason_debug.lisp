...
;;   - Behandlung von arithmetischen Ausdr√ºcken
...
(defun s (la)
  (cond ((la-check la '(forall var))
	...
	((la-check la NIL)
	 '(nil nil 3))
	(T (error "s: no fitting production"))))
...
(defun v (la)
  (cond ((la-check la '(var var))
	...
	((la-check la '(var in))
	 '((var in) nil 5))
	(T (error "v: no fitting production"))))
...
(defun k (la)
  (cond ((la-check la '(lit <=))
	...
	((la-check la '(lit =>))
	 '(() (f) 9))
	(T (error "k: no fitting production"))))
...
(defun f (la)
  (cond ((la-check la '(lit end))
	...
	((la-check la '(lit and))
	 '((lit and) (f) 7))
	(T (error "f: no fitting production"))))
...
(defun f2 (la)
  (cond ((la-check la '(lit end))
	...
	((la-check la '(lit and))
	 '((lit and) (f) 7))
	(T (error "f2: no fitting production"))))
...
(yacc:define-parser *term-parser*
  (:start-symbol S)
...
(defun parse-terms-in-lit (lit)
  (let ((term-list (get lit 'args)))
    (setf (get lit 'args) (condense-terms
			   (yacc:parse-with-lexer (make-term-lexer term-list)
					     *term-parser*)))
    lit))
...
;; Update: sollte irelevant sein, da mit antwort-pr√§dikat gearbeitet wird
...
;; (setq test-input (parse-file "C:/Users/Tobias/Documents/Studium/Teamprojekt/test.clr") input (multiple-value-list (split-prog-from-goals test-input)) program (car input) goal (car (cadr input)))
...
;Pr‰dikate pr¸fen und rauschmeiﬂen oder fail (nur bei cons nicht bei vars)
         ; (print "cur-procedure:")
	   (print cur-procedure)
           (mapcar (lambda (clause) 
(let ((neg-lits  (get clause 'cl-reason::neg-lits))) 
(mapcar (lambda (lit) (print (list '- (get lit 'cl-reason::name)
                       (mapcar 'bla (get lit 'cl-reason::args))))) neg-lits)
)
(let ((pos-lit  (get clause 'cl-reason::pos-lit))) 
 (print (list (get pos-lit 'cl-reason::name)
                        (mapcar 'bla (get pos-lit 'cl-reason::args))))
)
) cur-procedure)
(print "  ")
...
;(setq file-path "test_prog_2.clr")

;(setq input (multiple-value-list (split-prog-from-goals (parse-file file-path))))
;	 (setq program (car input))
;	 (setq goals (cadr input))
;(setq goal (car goals))